...
class RabbitMQClient:
    ...
    def setup_infrastructure(self) -> bool:
        """Создание exchange, очередей и привязок"""
        try:
            if not self.channel:
                logger.error("Нет активного соединения с RabbitMQ")
                return False
            
            # Создание exchange для задач
            self.channel.exchange_declare(
                exchange=self.config.tasks_exchange_name,
                exchange_type=self.config.tasks_exchange_type,
                durable=True
            )
            logger.info(f"Tasks Exchange создан: {self.config.tasks_exchange_name}")
            
            # Создание exchange для ответов
            self.channel.exchange_declare(
            ...
            # Создание очередей для задач и привязок
    ...
    def get_all_queues_info(self) -> Dict[str, Dict[str, Any]]:
        """Получение информации о всех очередях"""
        info = {}
